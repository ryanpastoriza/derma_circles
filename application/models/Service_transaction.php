...
class Service_transaction extends My_Model
{
	...
	public function get_service_transactions($filters = array()) {
	...
	public function filter_service_transactions($filters = array()){
		...
		$this->db->where(['date(service_transaction.date_created) >=' =>  $filters['date_from'], 'date(service_transaction.date_created) <=' =>  $filters['date_to']]);

		if( isset($filters['facialist_id']) && $filters['facialist_id'] != ''){
			$this->db->where('service_transaction.therapist_id', $filters['facialist_id']);	
		}

		if( isset($filters['patient_id']) && $filters['patient_id'] != ''){
			$this->db->where('service_transaction.patient_id', $filters['patient_id']);	
		}
